...
# Clear out any old symlinks from previous attempts in the pending directory
rm -f "${PENDING_DIR}"/*.json

echo "Scanning for completed structures in ${BATCH_ID}..."
...
for input_file in "${INPUT_DIR}"/*.json; do
    ...
    # If the _data.json does NOT exist in the output directory, we need to process it
    if [ ! -f "${OUTPUT_DIR}/${base_id}_data.json" ]; then
...
# Note: Since PENDING_DIR is inside /pub/absara/datasets/ASD/af3, it is automatically covered by the -B bind mount
