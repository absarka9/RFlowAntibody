...
class RankFlow(LightningModule):
    ...
    def forward(self, x, structure_repr):
        ...
        if getattr(self, "use_residue_aligned", False):
            # Build boolean mask of physical residues (Shape: [B, T])
            res_mask = self._build_res_mask(tok)
            mask_1d = res_mask[0] 
            
            # 1. If representation includes special tokens (T), strip down to physical residues (L)
            if representation.shape[1] == tok.shape[1]:
                representation = representation[:, mask_1d, :]
                
            # 2. If attention includes special tokens (T x T), strip down to (L x L)
            if attention.shape[2] == tok.shape[1]:
                attention = attention[:, :, mask_1d, :][:, :, :, mask_1d]
                
            # 3. If structure_repr was padded to (T) by the validation loop, strip down to (L)
            if structure_repr.shape[1] == tok.shape[1]:
                structure_repr = structure_repr[:, mask_1d, :]

            # Now all three tensors are mathematically guaranteed to be length L
            L = representation.shape[1]
            residx = torch.arange(L, device=self.device).unsqueeze(0).expand(tok.shape[0], -1)
            mask = torch.ones(tok.shape[0], L, device=self.device)
        else:
            # Legacy single-chain logic
            residx = torch.arange(tok.shape[1], device=self.device).expand_as(tok)
            mask = torch.ones_like(tok)

        # The math will now flawlessly execute regardless of where the tensors came from:
    ...
    def loss_compute_and_backward(self, x, y, structure_repr, msa_bank, name=None):
        ...
        for chunk_idx in range(num_chunks):
            ...
            # -------------------
                ...
                # ---- FIX: Separate Token and Residue Rows ----
                token_rows_list = []
                residue_rows_list = []
                ...
                for (pos1, wt33, mut33) in item['mutants']:
                    ...
                    if self.use_residue_aligned:
                        ...
                        r_row = int(res_mask_tok[0, :pos1].sum().item())
                    else:
                        t_row = pos1 - 1
                        r_row = pos1 - 1
